...
- **Install.** On Android (Chrome) use the "Install app" / "Add to Home Screen"
  prompt; on iOS (Safari) use Share → "Add to Home Screen". It then launches
...
- **Offline (view).** A service worker (generated by
  ...
  their own — an "offline" banner shows while you're disconnected.
