...
func TestToPlanPartDTO_FlightTZFallbackAndEffectiveAt(t *testing.T) {
	...
	dto := ToPlanPartDTO(part, flight, nil, nil, nil, nil, nil, nil, nil)
...
func TestToPlanPartDTO_NonFlightTypesSelectTheRightDetail(t *testing.T) {
	...
	if d := ToPlanPartDTO(base("hotel"), nil, &store.HotelDetail{PropertyName: "H"}, nil, nil, nil, nil, nil, nil); d.Hotel == nil || d.Hotel.PropertyName != "H" {
		t.Error("hotel detail not projected")
	}
	if d := ToPlanPartDTO(base("train"), nil, nil, &store.TrainDetail{Operator: "Eurostar"}, nil, nil, nil, nil, nil); d.Train == nil || d.Train.Operator != "Eurostar" {
		t.Error("train detail not projected")
	}
	if d := ToPlanPartDTO(base("ground"), nil, nil, nil, &store.GroundDetail{Provider: "Uber"}, nil, nil, nil, nil); d.Ground == nil || d.Ground.Provider != "Uber" {
		t.Error("ground detail not projected")
	}
	if d := ToPlanPartDTO(base("dining"), nil, nil, nil, nil, &store.DiningDetail{ReservationName: "R"}, nil, nil, nil); d.Dining == nil || d.Dining.ReservationName != "R" {
		t.Error("dining detail not projected")
	}
	if d := ToPlanPartDTO(base("excursion"), nil, nil, nil, nil, nil, &store.ExcursionDetail{Provider: "Tour"}, nil, nil); d.Excursion == nil || d.Excursion.Provider != "Tour" {
		t.Error("excursion detail not projected")
	}
	// Non-flight effective_at is just StartsAt.
	d := ToPlanPartDTO(base("dining"), nil, nil, nil, nil, &store.DiningDetail{}, nil, nil, nil)
