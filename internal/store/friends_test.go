...
// The legacy flight-grant cleanup tests (TestRemoveFriendshipDropsCrossShares /
// TestRemoveFriendshipPendingLeavesGrantsAlone) were removed in Wave 3: unfriending
// no longer touches any flight grants now that the flights / flight_passengers /
// flight_shares tables are gone. Plan-level visibility is trip-membership based
// and unaffected by unfriending, so there is nothing left here to assert.
