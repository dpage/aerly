...
/** The core slice: auth/me/capabilities, the user + friendship caches, the
 * notification badge, and the (legacy, Wave-3-bound) flight state. Kept intact
 * so the existing app keeps working while the trip-planning slices are filled
 * in by feature waves. */
...
export const createCoreSlice: StateCreator<StoreState, [], [], CoreSlice> = (set, get) => ({
  ...
  async refreshAll() {
    await Promise.all([get().refreshFlights(), get().refreshUsers(), get().refreshFriendships()]);
  },
