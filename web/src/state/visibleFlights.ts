 ...
 * honouring the user's `showOld` toggle and ageing flights out as time
 * passes (refreshed every OLD_TICK_MS). */
export function useVisibleFlights(): Flight[] {
  ...
  return useMemo(() => {
    if (showOld) return flights;
    return flights.filter((f) => !isOld(f, now));
  }, [flights, showOld, now]);
}
